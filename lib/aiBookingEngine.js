...
// Implements Custom Quotation Gathering for Chauffeur Services + Fixed Transparent Tariff for Self-Drive
...
/**
 * Calculates custom estimated quote / fixed self-drive tariff, and generates multilingual AI quotation response
...
export function generateAiQuote(parsed) {
  ...
  let isFixedTariff = false;
  let fareTypeLabel = 'Estimated Starting Fare';
  let promoApplicable = false;
  let promoMessage = '';
  let specialGuidance = null;

  // Key factors note explaining why final custom quotation may vary
  const variableFactorsNote = 'Final price is customized based on exact pickup/drop points, intermediate stops, toll charges, driver allowance, and duration.';
  ...
  if (serviceCategory === 'partner') {
    ...
    estimatedFare = 35000;
    fareTypeLabel = 'Monthly Earnings Estimate';
    ...
    estimatedFare = 0;
    fareTypeLabel = 'Free Inspection & Valuation';
    ...
    // Self-Drive is standardized / fixed daily tariff
    isFixedTariff = true;
    fareTypeLabel = 'Standard Daily Rental (24 Hours)';
    basePrice = 1499;
    ...
    tripTypeDisplay = '8 Hours / 80 km City Package (Est. Base)';
    estimatedFare = 1799;
  }

  // 2. Generate WhatsApp Custom Quotation Payload
  const waLines = [
    isFixedTariff
      ? `*MANA Tours & Travels — Self-Drive Booking Request*`
      : `*MANA Tours & Travels — Custom Trip Quotation Enquiry*`,
    `----------------------------------------`,
    `📍 *Route / Service:* ${routeLabel}`,
    `🚗 *Vehicle Preference:* ${matchedVehicle.name} (${matchedVehicle.seats})`,
    `📋 *Trip Plan:* ${tripTypeDisplay}`,
    `👥 *Passengers / Group:* ${passengers} Persons`,
    timeNote ? `⏰ *Preferred Timing:* ${timeNote}` : null,
    estimatedFare > 0
      ? isFixedTariff
        ? `💰 *Standard Tariff:* ₹${estimatedFare.toLocaleString('en-IN')}/day`
        : `💰 *Estimated Base:* Starts from ₹${estimatedFare.toLocaleString('en-IN')}`
      : null,
    promoApplicable ? `🎁 *Deal Requested:* ${promoMessage}` : null,
    `----------------------------------------`,
    `*Inclusions:* ${inclusions.slice(0, 2).join(', ')}`,
    !isFixedTariff
      ? `*Customer Note:* Please review my stops, tolls, and timings to share the exact custom quotation.`
      : `*Customer Note:* Please verify car availability for my dates.`,
  ].filter(Boolean);
  ...
  if (serviceCategory === 'partner') {
    ...
    responseTextTelugu = `నమస్కారం! MANA సెల్ఫ్ డ్రైవ్ కార్లు రోజుకు ₹${estimatedFare.toLocaleString('en-IN')} స్టాండర్డ్ రేటుతో ప్రారంభమవుతాయి. ${
    ...
    responseTextTelugu = `నమస్కారం! కడప నుండి ${matchedDest.name} కి ${matchedVehicle.name} లో సుమారు బేస్ ఫేర్ ₹${estimatedFare.toLocaleString('en-IN')} నుండి ప్రారంభమవుతుంది. మీ ఖచ్చితమైన స్టాప్‌లు, టోల్ గేట్లు, వేచి ఉండే సమయం మరియు ప్రయాణీకుల సంఖ్యను బట్టి పూర్తి కొటేషన్ ఇస్తాము. వాట్సాప్‌లో కస్టమ్ కొటేషన్ కోసం క్రింది బటన్ నొక్కండి.`;

    responseTextEnglish = `Namaste! For Kadapa to ${matchedDest.name} in a ${matchedVehicle.name}, the estimated base fare starts from ₹${estimatedFare.toLocaleString('en-IN')}. The exact quote is customized based on your specific pickup points, intermediate stops, toll charges, waiting duration, and passenger requirements. Tap below to get your custom quote on WhatsApp!`;

    responseTextHindi = `नमस्ते! कडपा से ${matchedDest.name} के लिए ${matchedVehicle.name} का अनुमानित बेस किराया ₹${estimatedFare.toLocaleString('en-IN')} से शुरू होता है। टोल, स्टॉप और समय के आधार पर अंतिम कस्टम कोटेशन व्हाट्सएप पर प्राप्त करें।`;
  } else {
    responseTextTelugu = `నమస్కారం! MANA టూర్స్ & ట్రావెల్స్ కడప కి స్వాగతం. తిరుపతి దర్శనం, హైదరాబాద్/బెంగళూరు ఎయిర్‌పోర్ట్ డ్రాప్స్, గండికోట టూర్స్, మరియు సెల్ఫ్ డ్రైవ్ కార్ల వివరాల కోసం మాట్లాడండి లేదా టైప్ చేయండి. మీ అవసరాలకు తగిన కస్టమ్ కొటేషన్ అందిస్తాము.`;

    responseTextEnglish = `Hello! Welcome to MANA Tours & Travels Kadapa. We understand your exact trip requirements (passengers, timing, route, tolls, and stops) to provide a tailored custom quotation. How can we assist your journey today?`;
  }
