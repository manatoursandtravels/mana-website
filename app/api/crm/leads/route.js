...
export async function GET(request) {
  ...
  try {
    // 1. If we have the Webhook URL, fetch live data securely from Google Apps Script
    if (webhookUrl && webhookUrl.startsWith('http')) {
      try {
        ...
        console.warn('Webhook GET error, falling back to GViz:', webhookErr.message);
      }
    }

    // 2. Fallback: Query Google Visualization JSON feed
